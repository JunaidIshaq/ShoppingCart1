...
export class FilterService {
  ...
  filterItemsBySearchText(items: IItem[], searchText: string): IItem[] {
    ...
    const resultItems: IItem[] = items.filter(item => {
  ...
  filterOrdersBySearchText(orders: IOrder[], searchText: string): IOrder[] {
    ...
    // Remove all white spaces
    searchText = searchText.toLowerCase().replace(/\s/g, '');
    const resultOrders: IOrder[] = orders.filter(order => {
      const orderDetails: IOrderDetail[] = order.orderDetails;
      let fullDescription = this.transformService.transformOrderDetailsToText(orderDetails);

      const dateDescription = this.transformService.transformDateToText(order.orderDate);
  ...
  filterElementsToDisplayPerPage(elements: any[], pageSize: number, event?: PageEvent): any[] {
    ...
    if (event === undefined || event == null) {
      ...
      const resultElements: any[] = elements.slice(0, pageSize);
      return resultElements;
    }

    const currentPageIndex: number = event.pageIndex;
    ...
    const resultElements: any[] = elements.slice(startIndex, endIndex);
